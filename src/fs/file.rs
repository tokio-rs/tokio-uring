...
pub type RwFlags = io_uring::types::RwFlags;
...
impl File {
    ...
    pub async fn read_at<T: BoundedBufMut>(&self, buf: T, pos: u64) -> crate::BufResult<usize, T> {
        ...
        let op = Op::read_at(&self.fd, buf, pos, 0).unwrap();
        op.await
    }
    
    /// Read some bytes at the specified offset from the file into the specified
    /// buffer, returning how many bytes were read, and allows the caller to
    /// specify input/output operation flags
    ///
    /// # Return
    ///
    /// The method returns the operation result and the same buffer value passed
    ...
    /// }
    /// ```
    pub async fn read_at_with_flags<T: BoundedBufMut>(&self, buf: T, pos: u64, flags: RwFlags) -> crate::BufResult<usize, T> {
        // Submit the read operation
        let op = Op::read_at(&self.fd, buf, pos, flags).unwrap();
        op.await
    }

    /// Read some bytes at the specified offset from the file into the specified
    /// array of buffers, returning how many bytes were read, and allows the caller to
    /// specify input/output operation flags.
    ///
    /// # Return
    ///
    ...
    /// }
    /// ```
    pub async fn readv_at_with_flags<T: BoundedBufMut>(
        &self,
        bufs: Vec<T>,
        pos: u64,
        flags: RwFlags,
        ...
        // Submit the read operation
        let op = Op::readv_at(&self.fd, bufs, pos, flags).unwrap();
        op.await
    }
    ...
    pub async fn readv_at<T: BoundedBufMut>(
        ...
        // Submit the read operation
        let op = Op::readv_at(&self.fd, bufs, pos, 0).unwrap();
        op.await
    }
    ...
    pub async fn writev_at<T: BoundedBuf>(
        ...
        let op = Op::writev_at(&self.fd, buf, pos, 0).unwrap();
        op.await
    }
    ...
    /// error. The bytes will be written starting at the specified offset.
    ///
    /// This functions allows the caller to specify I/O operation flags,
    /// described in ```man preadv2(2)```.
    ///
    /// # Return
    ///
    /// The method returns the operation result and the same array of buffers passed
    ...
    ///
    /// [`Ok(n)`]: Ok
    pub async fn writev_at_with_flags<T: BoundedBuf>(
        &self,
        buf: Vec<T>,
        pos: u64,
        flags: RwFlags,
    ) -> crate::BufResult<usize, Vec<T>> {
        let op = Op::writev_at(&self.fd, buf, pos, flags).unwrap();
        op.await
    }
    ...
    pub async fn writev_at_all<T: BoundedBuf>(
        ...
        let op = crate::io::writev_at_all(&self.fd, buf, pos, 0);
        op.await
    }
    
    /// Like `writev_at_with_flags` but will call the `io_uring` `writev` operation multiple times if
    /// necessary.
    ///
    /// Parameter `pos` is an `Option<u64>` to allow this function to be used for both files that
    /// are seekable and those that are not. The caller is responsible for knowing this.
    ...
    /// TODO consider replacing the current `write_all` and `write_all_at` functions with a similar
    /// mechanism so all the write-all logic is in one place, in the io/write_all.rs file.
    pub async fn writev_at_all_with_flags<T: BoundedBuf>(
        &self,
        buf: Vec<T>,
        pos: Option<u64>, // Use None for files that can't seek
        flags: RwFlags,
    ) -> crate::BufResult<usize, Vec<T>> {
        let op = crate::io::writev_at_all(&self.fd, buf, pos, flags);
        op.await
    }
        ...
        T: BoundedBufMut,
    {
        ...
        let (res, buf) = self.read_exact_slice_at(buf.slice_full(), pos, 0).await;
        (res, T::from_buf_bounds(buf, orig_bounds))
    }
    
    /// Read the exact number of bytes required to fill `buf` at the specified
    /// offset from the file, and allows the caller to specify I/O operation flags.
    ///
    /// This function reads as many as bytes as necessary to completely fill the
    /// specified buffer `buf`.
    ///
    ...
    ///
    /// [`ErrorKind::UnexpectedEof`]: std::io::ErrorKind::UnexpectedEof
    pub async fn read_exact_at_with_flags<T>(&self, buf: T, pos: u64, flags: RwFlags) -> crate::BufResult<(), T>
    where
        T: BoundedBufMut,
    {
        let orig_bounds = buf.bounds();
        let (res, buf) = self.read_exact_slice_at(buf.slice_full(), pos, flags).await;
        (res, T::from_buf_bounds(buf, orig_bounds))
    }
    ...
    async fn read_exact_slice_at<T: IoBufMut>(
        ...
        while buf.bytes_total() != 0 {
            let (res, slice) = self.read_at_with_flags(buf, pos, flags).await;
            match res {
                Ok(0) => {
                    return (
                        Err(io::Error::new(
    ...
    {
        // Submit the read operation
        let op = Op::read_fixed_at(&self.fd, buf, pos, 0).unwrap();
        op.await
    }
    
    /// Like [`read_at`], but using a pre-mapped buffer
    /// registered with [`FixedBufRegistry`] and allows the caller
    /// to specify I/O operation flags.
    ///
    /// [`read_at`]: Self::read_at
    /// [`FixedBufRegistry`]: crate::buf::fixed::FixedBufRegistry
    ///
    ...
    ///# }
    /// ```
    pub async fn read_fixed_at_with_flags<T>(&self, buf: T, pos: u64, flags: RwFlags) -> crate::BufResult<usize, T>
    where
        T: BoundedBufMut<BufMut = FixedBuf>,
    {
        // Submit the read operation
        let op = Op::read_fixed_at(&self.fd, buf, pos, flags).unwrap();
        op.await
    }
    ...
    pub fn write_at<T: BoundedBuf>(&self, buf: T, pos: u64) -> UnsubmittedWrite<T> {
        UnsubmittedOneshot::write_at(&self.fd, buf, pos, 0)
    }
    
    /// Write a buffer into this file at the specified offset, returning how
    /// many bytes were written, and allows the caller to specify the I/O
    /// operation flags.
    ///
    /// This function will attempt to write the entire contents of `buf`, but
    /// the entire write may not succeed, or the write may also generate an
    ...
    ///
    /// [`Ok(n)`]: Ok
    pub fn write_at_with_flags<T: BoundedBuf>(&self, buf: T, pos: u64, flags: RwFlags) -> UnsubmittedWrite<T> {
        UnsubmittedOneshot::write_at(&self.fd, buf, pos, flags)
    }
    ...
    {
        let orig_bounds = buf.bounds();
        let (res, buf) = self.write_all_slice_at(buf.slice_full(), pos, 0).await;
        (res, T::from_buf_bounds(buf, orig_bounds))
    }
    
    /// Attempts to write an entire buffer into this file at the specified offset,
    /// but allows the caller to specify I/O operation flags.
    ///
    /// This method will continuously call [`write_at`] until there is no more data
    /// to be written or an error is returned.
    /// This method will not return until the entire buffer has been successfully
    ...
    ///
    /// [`write_at`]: File::write_at
    pub async fn write_all_at_with_flags<T>(&self, buf: T, pos: u64, flags: RwFlags) -> crate::BufResult<(), T>
    where
        T: BoundedBuf,
    {
        let orig_bounds = buf.bounds();
        let (res, buf) = self.write_all_slice_at(buf.slice_full(), pos, flags).await;
        (res, T::from_buf_bounds(buf, orig_bounds))
    }
    ...
    async fn write_all_slice_at<T: IoBuf>(
        ...
        while buf.bytes_init() != 0 {
            let (res, slice) = self.write_at_with_flags(buf, pos, flags).submit().await;
            match res {
                Ok(0) => {
                    return (
                        Err(io::Error::new(
    ...
    {
        let op = Op::write_fixed_at(&self.fd, buf, pos, 0).unwrap();
        op.await
    }
    
    /// Like [`write_at`], but using a pre-mapped buffer
    /// registered with [`FixedBufRegistry`] and allows specifying
    /// an input-output operation flags (see ```man preadv2(2)```)
    ///
    /// [`write_at`]: Self::write_at
    /// [`FixedBufRegistry`]: crate::buf::fixed::FixedBufRegistry
    ///
    ...
    ///# }
    /// ```
    pub async fn write_fixed_at_with_flags<T>(&self, buf: T, pos: u64, flags: RwFlags) -> crate::BufResult<usize, T>
    where
        T: BoundedBuf<Buf = FixedBuf>,
    {
        let op = Op::write_fixed_at(&self.fd, buf, pos, flags).unwrap();
        op.await
    }
    ...
    {
        let orig_bounds = buf.bounds();
        let (res, buf) = self.write_fixed_all_at_slice(buf.slice_full(), pos, 0).await;
        (res, T::from_buf_bounds(buf, orig_bounds))
    }
    
    /// Attempts to write an entire buffer into this file at the specified offset.
    /// Also, allows specifying input/output operation flags.
    ///
    /// This method will continuously call [`write_fixed_at`] until there is no more data
    /// to be written or an error is returned.
    /// This method will not return until the entire buffer has been successfully
    ...
    ///
    /// [`write_fixed_at`]: Self::write_fixed_at
    pub async fn write_fixed_all_at_with_flags<T>(&self, buf: T, pos: u64, flags: RwFlags) -> crate::BufResult<(), T>
    where
        T: BoundedBuf<Buf = FixedBuf>,
    {
        let orig_bounds = buf.bounds();
        let (res, buf) = self.write_fixed_all_at_slice(buf.slice_full(), pos, flags).await;
        (res, T::from_buf_bounds(buf, orig_bounds))
    }
    ...
    async fn write_fixed_all_at_slice(
        ...
        while buf.bytes_init() != 0 {
            let (res, slice) = self.write_fixed_at_with_flags(buf, pos, flags).await;
            match res {
                Ok(0) => {
                    return (
                        Err(io::Error::new(
