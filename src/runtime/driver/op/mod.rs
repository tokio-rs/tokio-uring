...
mod test {
    ...
    fn complete_after_drop() {
        ...
        CONTEXT.with(|cx| cx.with_handle_mut(|driver| driver.ops.complete(index, cqe)));
    ...
    fn init() -> (Op<Rc<()>>, Rc<()>) {
        ...
        let op = CONTEXT.with(|cx| {
            cx.set_handle(driver);

            cx.with_handle_mut(|driver| Op::new(data.clone(), driver))
        });
    ...
    fn num_operations() -> usize {
        CONTEXT.with(|cx| cx.with_handle_mut(|driver| driver.num_operations()))
    }

    fn complete(op: &Op<Rc<()>>, result: io::Result<u32>) {
        let cqe = CqeResult { result, flags: 0 };
        CONTEXT.with(|cx| cx.with_handle_mut(|driver| driver.ops.complete(op.index, cqe)));
    }
